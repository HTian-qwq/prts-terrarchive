...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const citedArchiveIds = (sources, answer, wikiCitations) => {
      const cited = new Set()
      const candidates = mergeArchiveSources(sources)
      ...
      for (const match of answer.matchAll(/《([^》\n]+)》(?:（document_uid=(doc_[A-Za-z0-9_-]{16})）)?\s*(?:第\s*(\d+)(?:\s*[-—–~]\s*(\d+))?\s*行|Wiki·([^\s。；，,;\n]+))/gu)) {
        ...
        const hits = candidates.filter((source) => source.title === match[1]
          && (!match[2] || source.documentUid === match[2]))
        // Even UID alone cannot disambiguate two revisions of the same document.
        if (hits.length !== 1) continue
        const source = hits[0]
        if (match[5] ? wikiCitations?.get(source.id)?.has(match[5])
          : Number.isSafeInteger(start) && start > 0 && end >= start && matchesRange(source, start, end)) {
          cited.add(source.id)
        }
      }
      for (const source of candidates) {
        if (!source.sourceRef || typeof source.sourceRef !== 'string') continue
        const escaped = source.sourceRef.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')
        if (new RegExp(`(?:^|[\\s\\x60("'=:])${escaped}(?=$|[\\s\\x60)"'\\],.;，。；])`, 'u').test(answer)) {
          const hits = candidates.filter((item) => item.sourceRef === source.sourceRef)
          if (hits.length === 1) cited.add(source.id)
        }
      }
      return cited
    }
    const isRhineReadTool = (name) => name === 'corpus_read' || name === 'cloud_inspect'
    ...
    const buildRhineSnapshot = (order = [], nodes, sessionId = '', sessionRunning = false, timeline) => {
      const sources = []
      const records = []
      const operations = []
      ...
      for (const [key, node] of entries) {
        if (node?.kind === 'user' || node?.kind === 'steering') {
          ...
          if (!currentTurn) { turnSourceStart = sources.length; operations.length = 0; operationSources.clear() }
        ...
        if (node?.kind === 'assistant-step') {
          ...
          if (text) {
            ...
            for (const id of citedArchiveIds(sources, text, wikiCitations)) cited.add(id)
          }
        ...
        if (!call || !PRTS_SCENE_TOOLS.includes(call.name)) continue
        const meta = rhineToolPresentation(root)
        if (typeof meta?.error === 'string' && meta.error) call.state = 'error'
        const args = parseToolArguments(call.argsRaw)
        ...
        const currentQuery = String(args.query || args.title || locator.display_title || locator.document_id
          || locator.document_uid || locator.source_ref || args.activity_name || args.collection_name || args.section || '')
        const text = root?.kind === 'tool-result' ? contentText(root.content) : ''
        const current = belongsToCurrentTurn(node)
        records.push({ id: String(key), tool: call.name, state: call.state, query: currentQuery, text })
        if (current) {
        ...
        if (meta?.kind === 'prts-archive-sources-v1' && Array.isArray(meta.sources)) {
          sources.push(...meta.sources.slice(0, 128).map((source) => ({ ...source, callId: String(key) })))
        }
        if (call.name === 'corpus_read') {
          ...
          for (const source of readSources) {
            ...
            for (const citation of text.matchAll(/^引用：《([^》\n]+)》Wiki·([^\s\n]+)$/gmu)) {
              if (citation[1] !== (source.title || read.title)) continue
              if (!wikiCitations.has(id)) wikiCitations.set(id, new Set())
              wikiCitations.get(id).add(citation[2])
            }
            sources.push({ ...source,
              id,
      ...
      const materials = mergeArchiveSources(sources).map((source) => cited.has(source.id)
        ? { ...source, state: 'cited' } : source)
      ...
      let outcome = sessionRunning || searching || assistantRunning ? 'running'
        : terminalError ? 'error' : interrupted ? 'interrupted' : 'unknown'
      if (endKind) {
      ...
      for (const source of materials) {
        for (const alias of sourceAliases(source)) {
          if (!materialIds.has(alias)) materialIds.set(alias, new Set())
          materialIds.get(alias).add(source.id)
        }
      }
      ...
      const reportIds = citedArchiveIds(materials, answer, wikiCitations)
      const reportSources = materials.filter(source => reportIds.has(source.id)).map(source => ({ ...source, state: 'cited' }))
      const currentSources = mergeArchiveSources(currentTurn ? turnSources : sources.slice(turnSourceStart))
        .map(source => reportIds.has(source.id) ? { ...source, state: 'cited' } : source)
      ...
      const snapshot = { sessionId, running: !endKind && Boolean(sessionRunning || searching || assistantRunning), searching, question, query, tool,
        ...
        sources: materials, answer, records, operations, ...(error ? { error } : {}),
      ...
      return { ...snapshot, phase: rhinePhase(snapshot) }
    }
    const rhineSnapshotSignature = (snapshot) => JSON.stringify(buildRhineSnapshot(snapshot?.order, snapshot?.nodes, '', false, snapshot?.timeline))
    ...
    function createRhineHostControls(ctx, sessionId) {
      ...
      for (const store of [sessions.list, ctx.workspaces?.list, session, directory?.store]) {
        if (store?.subscribe) cleanups.push(store.subscribe(emit))
      }
      const controls = {
        ...
        dispose() { disposed = true; pendingCreated = null; for (const cleanup of cleanups) cleanup(); listeners.clear() },
      }
      return controls
    }
    function RhineControl({ useChat, useSession, sessionId }) {
      ...
      const serialized = useChat ? useChat(rhineSnapshotSignature) : ''
      ...
      const snapshot = useMemo(() => {
        const next = serialized ? JSON.parse(serialized) : buildRhineSnapshot([], null)
        const previous = previousSources.current
        const sources = mergeArchiveSources([...(previous.sessionId === sessionId ? previous.sources : []), ...next.sources])
        previousSources.current = { sessionId, sources }
      ...
      }, [serialized, sessionId, sessionState])
      ...
      useEffect(() => {
        ...
        void loadRhineBundle().then((runtime) => {
          ...
          instance = runtime.mountRhineWorkbench(overlay, {
            assetBase: RHINE_BASE, snapshot: latestSnapshot.current,
    ...
    exports.__rhineStateForTest = { buildRhineSnapshot, mergeArchiveSources, rhineSnapshotSignature, submitRhineQuestion, RhineControl }
