/**
 * prts-terrarchive 浏览器宿主适配器（手写）；莱茵资料馆单独编译并按需加载。
 *
 ...
 * 数据：Host Connection 共享 Fetch 通道 /api/prts-corpus/rpc（Web / Electron）。
 */
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function isHostSettingsOpen(doc = document) {
      return Boolean(doc.querySelector('[data-shortcut-modal="settings"], '
        + '[data-slot="sidebar.settings"] [role="dialog"][aria-modal="true"], '
        + '[role="dialog"][aria-modal="true"]:has([data-slot="settings.header"])'))
    }

    function openHostSettings(ctx = clientContext, doc = document) {
      if (isHostSettingsOpen(doc)) return true
      // DSH 0.1.3/0.1.5 keeps settings state inside its mounted React component.
      // Clicking its own trigger preserves that instance (including hidden sidebars).
      const trigger = doc.querySelector('[data-slot="sidebar.settings"] button[aria-haspopup="dialog"]')
      if (trigger && !trigger.disabled) {
        trigger.click()
        return true
      }
      // DSH 0.1.7 replaces the trigger with settings.launcher (account menu).
      // Its registered shell store's create() returns the SAME instance used by
      // SettingsRoot and settings.open. Read the live winning entry on each click;
      // retaining it would target a dead shell after a host/plugin reload.
      const slots = ctx?.slots
      const entries = slots?.entriesOfSlot?.('sidebar.settings') ?? slots?.entries?.('sidebar.settings') ?? []
      const shell = entries[0]
      if (!shell?.children?.['settings.launcher'] || typeof shell.store?.create !== 'function') return false
      const instance = shell.store.create()
      if (typeof instance?.actions?.open !== 'function') return false
    ...
    function aicBootShow(message = 'INITIALIZING', restart = false, skin = 'endfield-aic') {
      ...
      if (!document.getElementById('aic-boot-style')) {
        ...
        style.textContent = '.aic-boot{position:fixed;inset:0;z-index:99999;background:#0b0d10;display:flex;align-items:center;justify-content:center;transition:opacity .6s ease .15s,visibility .6s ease .15s}'
          + '.aic-boot[data-skin="prts-agent"],.aic-boot[data-skin="harness"]{background:#f5f5f1;color:#202327}'
          + '.aic-boot[data-skin="rhine-lab"]{background:#ebe8e2;color:#30362c}'
          + '.aic-boot:not([data-skin="endfield-aic"]) .aic-boot-logo{color:inherit;letter-spacing:.12em}'
          + '.aic-boot:not([data-skin="endfield-aic"]) :is(.aic-boot-sub,.aic-boot-line){color:#676b62}'
          + '.aic-boot:not([data-skin="endfield-aic"]) .aic-boot-bar{background:#a5a99655}'
          + '.aic-boot:not([data-skin="endfield-aic"]) .aic-boot-fill{background:currentColor;box-shadow:none}'
      ...
      aicBootEl.dataset.skin = skin
      const branding = skin === 'endfield-aic' ? ['AIC', 'ENDFIELD INDUSTRIES // TALOS-II SURVEY SYSTEM']
        : skin === 'rhine-lab' ? ['RHINE LAB', 'SYNTHESIZE INFORMATION // ARCHIVE SYSTEM']
        : skin === 'prts-agent' ? ['PRTS', 'TERRARCHIVE // RETRIEVAL SYSTEM']
        : ['HARNESS', 'PREPARING WORKSPACE']
      aicBootEl.innerHTML = `<div class="aic-boot-box"><div class="aic-boot-logo">${branding[0]}</div>`
        + `<div class="aic-boot-sub">${branding[1]}</div>`
      ...
      document.documentElement.style.background = skin === 'endfield-aic' ? '#0b0d10' : skin === 'rhine-lab' ? '#ebe8e2' : '#f5f5f1'
      aicBootPct = 4
      aicBootFinishing = false
      aicBootPaint(aicBootPct, message, token)
    ...
    // Match the host's settings and menus to the same ivory/olive workspace.
    const RHINE_LAB_TOKENS = Object.freeze({
      ...PRTS_AGENT_TOKENS,
      '--dsw-alias-bg-base': canonical('#ebe8e2'),
      '--dsw-alias-bg-layer-1': canonical('#e7e3d9'),
      '--dsw-alias-bg-layer-2': canonical('#f3f0e8'),
      '--dsw-alias-bg-layer-3': canonical('#faf8f1'),
      '--dsw-alias-bg-mask-1': canonical('rgba(39,43,33,.32)'),
      '--dsw-alias-brand-primary': canonical('#59634b'),
      '--dsw-alias-brand-text': canonical('#48533d'),
      '--dsw-alias-label-primary': canonical('#282e24'),
      '--dsw-alias-label-secondary': canonical('#505747'),
      '--dsw-alias-label-tertiary': canonical('#69705e'),
      '--dsw-alias-label-caption': canonical('#717664'),
      '--dsw-alias-button-info-fill': canonical('#59634b'),
      '--dsw-alias-button-info-hover': canonical('#454e39'),
      '--dsw-alias-button-elevated-fill': canonical('#f3f0e8'),
      '--dsw-alias-button-floating-fill': canonical('#f3f0e8'),
      '--dsw-alias-interactive-bg-active': canonical('rgba(99,112,77,.14)'),
      '--dsw-alias-interactive-bg-hover': canonical('rgba(99,112,77,.08)'),
      '--dsw-alias-state-business-primary': canonical('#59634b'),
      '--dsw-alias-state-business-tertiary': canonical('#e3e6d7'),
      '--dsw-specific-menu': canonical('#f3f0e8'),
      '--dsw-specific-sidebar-fill': canonical('#e7e3d9'),
    })

    const normalizeSkin = (skin) => skin === 'prts-agent' || skin === 'endfield-aic' || skin === 'rhine-lab'
      ? skin : 'harness'
    const skinTokens = (skin) => skin === 'prts-agent' ? PRTS_AGENT_TOKENS
      : skin === 'endfield-aic' ? ENDFIELD_AIC_TOKENS : skin === 'rhine-lab' ? RHINE_LAB_TOKENS : null
    const writeSkinDataset = (skin) => {
      if (typeof document !== 'undefined') {
        ...
        else if (skin === 'rhine-lab') document.body.dataset.prtsSkin = 'rhine-lab'
        else delete document.body.dataset.prtsSkin
      }
      for (const listener of rhineSkinListeners) listener(skin)
    }
    ...
    const buildSceneSnapshotModel = (order, nodes) => {
      ...
      return {
        ...
        stackStates: [
          visuals.cloud_search.state,
          visuals.corpus_search.state,
          visuals.corpus_read.state,
        ],
    ...
    const SCENE_HTML = `
      ...
      <div class="prts-trace-stack"><span data-state="standby">01&nbsp;&nbsp;cloud.search</span><span data-state="standby">02&nbsp;&nbsp;corpus.search</span><span data-state="standby">03&nbsp;&nbsp;source.read</span></div>
    ...
    const installScene = () => {
      ...
      // installScene 比 Conversation slot 更早挂载时先保持隐藏，避免在设置、
      // 工作区等非会话页面闪出开机场景。
      scene.dataset.page = 'hidden'
      ...
      const applySnapshotModel = (model, search, read) => {
        ...
        resolved.stack.forEach((text, index) => {
          if (!stack[index]) return
          stack[index].textContent = text
      ...
      const sync = () => {
        ...
        // 路由切走时 Conversation 会从 DOM 暂时卸载。它不代表新建了空白
        // 会话，不能把 scene 重置成 hero；否则返回已有会话会重放整套退场
        // 动画。这里只隐藏场景并保留阶段、计数与工具状态，等会话重新挂载。
        if (!conversation) {
          scene.dataset.page = 'hidden'
          return
        }
        scene.dataset.page = 'conversation'
    ...
    const SKIN_COMMON_STYLESHEET = Object.freeze({
      id: 'common', href: '/api/prts-corpus/skins/common.css',
    })
    const SKIN_STYLESHEETS = Object.freeze({
      'prts-agent': '/api/prts-corpus/skins/prts-agent.css',
      'endfield-aic': '/api/prts-corpus/skins/endfield-aic.css',
      'rhine-lab': '/api/prts-corpus/skins/rhine-lab.css',
    })
    ...
    const loadConfiguredSkin = async ({ signal, generation, skinEpoch, hostSyncGeneration }) => {
      ...
      if (typeof fetch === 'function') {
        try {
          const response = await fetchWithDeadline('/api/prts-corpus/ui-skin.json', { cache: 'no-store' }, { signal })
          assertCurrent()
          if (response.ok) {
    ...
    const validDataVersion = (value) => typeof value === 'string'
      && DATA_VERSION_PATTERN.test(value)

    /** 一条 corpus_read 模型投影 → 人类可读标题与引用范围。 */
    const readPresentation = (value) => {
      if (!value || value.kind !== 'prts-corpus-read-v1' || !value.locator
        || typeof value.locator !== 'object') return null
      const documentId = typeof value.locator.document_id === 'string'
        && value.locator.document_id.length <= 512 ? value.locator.document_id : ''
      const sourceRef = typeof value.locator.source_ref === 'string'
        && value.locator.source_ref.length <= 1024 ? value.locator.source_ref : ''
      const documentUid = typeof value.locator.document_uid === 'string'
        && /^doc_[A-Za-z0-9_-]{16}$/.test(value.locator.document_uid)
        ? value.locator.document_uid : ''
      if (!documentId && !sourceRef) return null
      const primary = {
        documentId, documentUid, sourceRef,
        // 只有契约中的 sha256 才能将原文绑定到确定版本。旧会话里的
        // 人类版本名或截断 hash 仅供展示，不能标记 VERIFIED。
        dataVersion: validDataVersion(value.data_version) ? value.data_version : '',
        title: typeof value.title === 'string' && value.title.length <= 512 ? value.title : '',
        lineStart: Number.isInteger(value.line_start) && value.line_start > 0 ? value.line_start : null,
        lineEnd: Number.isInteger(value.line_end) && value.line_end > 0 ? value.line_end : null,
      }
      const sources = Array.isArray(value.sources) ? value.sources.slice(0, 128).map((source) => {
        const sourceDocumentId = typeof source?.document_id === 'string'
          && source.document_id.length <= 512 ? source.document_id : ''
        if (!sourceDocumentId) return null
        const documentUid = typeof source.document_uid === 'string'
          && /^doc_[A-Za-z0-9_-]{16}$/.test(source.document_uid) ? source.document_uid : ''
        return { documentId: sourceDocumentId, documentUid, sourceRef: '', dataVersion: primary.dataVersion,
          title: typeof source.title === 'string' && source.title.length <= 512 ? source.title : '',
          lineStart: Number.isInteger(source.line_start) && source.line_start > 0 ? source.line_start : null,
          lineEnd: Number.isInteger(source.line_end) && source.line_end > 0 ? source.line_end : null }
      }).filter(Boolean) : []
      return { ...primary, sources }
    }

    const rememberReadBlock = (byTitle, full, rawPresentation) => {
      const presentation = readPresentation(rawPresentation)
      const titleMatch = full.match(/#\s+(.+?)\s+(?:范围：|字段：)/u)
      const citationMatch = full.match(/引用：《([^》]+)》(?:（document_uid=(doc_[A-Za-z0-9_-]{16})）)?(?:第\s*(\d+)(?:[-–](\d+))?\s*行|Wiki)/u)
      const projectedSources = presentation?.sources?.length ? presentation.sources : [presentation || {}]
      for (const source of projectedSources) {
        const title = String(source.title || presentation?.title || citationMatch?.[1]
          || titleMatch?.[1] || '').replace(/\s+/g, ' ').trim()
        if (!title) continue
        const lineStart = source.lineStart ?? (citationMatch?.[3] ? Number(citationMatch[3]) : null)
        const lineEnd = source.lineEnd ?? (citationMatch?.[4] ? Number(citationMatch[4]) : lineStart)
        const excerpt = presentation?.sources?.length > 1
          ? `《${title}》${lineStart ? `第 ${lineStart}${lineEnd && lineEnd !== lineStart ? `-${lineEnd}` : ''} 行` : '资料片段'}`
          : full.slice(0, 260)
        // 同一轮内可能因重试/热更新重读同名文档；按会话顺序保留最后一次，
        // 避免 first-write-wins 把旧 data_version 固化到证据卡。
        const documentUid = source.documentUid || citationMatch?.[2] || ''
        const key = documentUid ? `${title}\u0000${documentUid}` : title
        byTitle.set(key, { title, lineStart, lineEnd, excerpt, documentUid,
          documentId: source.documentId || '', sourceRef: source.sourceRef || '',
          dataVersion: source.dataVersion || presentation?.dataVersion || '' })
      }
    }
    ...
    const collectSnapshotEvidence = (order, nodes) => {
      ...
      // Header 标注的是“本轮证据”：从最后一条 user 节点开始投影，不把
      // 上一轮的 corpus_read 在当前回答无引用时回退进来。无 user 的旧快照
      // 保持兼容，仍按完整 order 处理。
      let turnStart = 0
      for (let index = 0; index < order.length; index += 1) {
        const node = typeof nodes.get === 'function' ? nodes.get(order[index]) : null
        if (node?.kind === 'user') turnStart = index
      }
      for (const key of order.slice(turnStart)) {
        ...
        if (node.kind === 'tool-call') {
          ...
          if (full.includes('引用：《')) rememberReadBlock(byTitle, full, root.meta)
        } else if (node.kind === 'assistant-step') {
    ...
    // ChatNodeStore and order retain their identities for same-key updates. Use
    // the evidence inputs themselves as the subscription value, including text
    // and metadata changes that do not alter the scene's status/text length.
    const evidenceSnapshotSignature = (snapshot) => {
      const { byTitle, lastAnswer } = collectSnapshotEvidence(snapshot?.order, snapshot?.nodes)
      return JSON.stringify({ reads: [...byTitle], lastAnswer })
    }
    ...
    const collectDomEvidence = () => {
      ...
      if (flowRows.length) {
        const turnStart = flowRows.findLastIndex((row) => row.dataset?.chatFlowKind === 'user')
        const currentRows = flowRows.slice(Math.max(0, turnStart))
        for (const row of currentRows) {
          const full = rowText(row)
          const isCorpusRead = row.matches?.('[data-tool="corpus_read"]')
            || row.querySelector?.('[data-tool="corpus_read"]') || full.includes('corpus_read')
          if (row.dataset?.chatFlowKind === 'tool-call' && isCorpusRead
              && full.includes('引用：《')) rememberReadBlock(byTitle, full)
          if (row.dataset?.chatFlowKind === 'assistant-step' && full
              && !full.includes('引用：《')) lastAnswer = full
        }
        return { byTitle, lastAnswer }
      }
      // 轨迹表回退：同样从最后一条 USER/用户消息开始。
      const rows = [...document.querySelectorAll('tr[data-kind]')]
      const turnStart = rows.findLastIndex((row) => row.dataset?.kind === 'message'
        && /USER|用户/i.test(row.querySelector('td[class*="event"]')?.textContent ?? ''))
      for (const row of rows.slice(Math.max(0, turnStart))) {
        const event = row.querySelector('td[class*="event"]')?.textContent ?? ''
        const full = rowText(row)
        if (row.dataset?.kind === 'tool' && full.includes('corpus_read')
            && full.includes('引用：《')) rememberReadBlock(byTitle, full)
        if (row.dataset?.kind === 'message' && /ASSISTANT/i.test(event)) lastAnswer = full
      }
      return { byTitle, lastAnswer }
    }
    ...
    const buildEvidence = ({ byTitle, lastAnswer }) => {
      ...
      const resolveTitle = (raw, uid = '') => {
        const title = String(raw ?? '').replace(/\s+/g, ' ').trim()
        const values = [...byTitle.values()]
        const exact = values.filter((item) => item.title === title
          && (!uid || item.documentUid === uid))
        if (exact.length === 1) return exact[0]
        if (title.length < 3) return null
        const candidates = values.filter((item) => item.title.length >= title.length
          && item.title.includes(title) && (!uid || item.documentUid === uid))
        if (candidates.length !== 1) return null
        return candidates[0]
      }
      const cited = new Map()
      // 行内可溯源引用：《标题》…第 N(-M)? 行。
      for (const m of lastAnswer.matchAll(/《([^》]+)》(?:（document_uid=(doc_[A-Za-z0-9_-]{16})）)?[^。；\n]{0,40}?第\s*(\d+)(?:\s*[-—–~]\s*(\d+))?\s*行/g)) {
        const hit = resolveTitle(m[1], m[2] || '')
        if (!hit) continue
        const lineStart = Number(m[3])
        const lineEnd = m[4] ? Number(m[4]) : lineStart
        cited.set(`${hit.title}\u0000${hit.documentUid || hit.documentId || ''}`,
          { ...hit, lineStart, lineEnd })
      }
      // 其余《标题》引用（如「主要参考资料」清单），一并纳入。
      for (const m of lastAnswer.matchAll(/《([^》]+)》(?:（document_uid=(doc_[A-Za-z0-9_-]{16})）)?/g)) {
        const hit = resolveTitle(m[1], m[2] || '')
        const key = hit ? `${hit.title}\u0000${hit.documentUid || hit.documentId || ''}` : ''
        if (hit && !cited.has(key)) cited.set(key, { ...hit, lineStart: hit.lineStart, lineEnd: hit.lineEnd })
      }
      // 回答里没有可映射引用时，仅回退到 collect* 已限定的本轮读取。
      ...
      return [...used.values()].map((item) => {
        ...
        return { id: `${index}-${signature.slice(0, 24)}`, title: item.title, kind: 'story',
          documentId: item.documentId || '', sourceRef: item.sourceRef || '', dataVersion: item.dataVersion || '',
          lineStart: item.lineStart, lineEnd: item.lineEnd,
          excerpt: item.excerpt || '已在回答中引用该资料，点开查看原文。' }
      }).filter(Boolean)
    }
    ...
    async function readEvidenceSource(item, cursor = '') {
      const locator = item.sourceRef ? { source_ref: item.sourceRef }
        : item.documentId ? { document_id: item.documentId } : null
      if (!locator) throw new Error('该证据缺少稳定的原文定位，请从当前会话重新读取')
      if (!validDataVersion(item.dataVersion)) {
        throw new Error('该证据没有完整的 64 位资料版本，为避免打开错版原文，请重新读取')
      }
      if (cursor && (typeof cursor !== 'string' || cursor.length > 65536)) {
        throw new Error('原文续页定位无效，请重新打开证据')
      }
      const result = await callApi('read', { locator,
        selection: { mode: 'document', ...(cursor ? { cursor } : {}) },
      ...
      if (result.response?.data_version !== item.dataVersion) {
        throw new Error('返回的原文版本与证据不匹配，已拒绝展示')
      }
      return result.response
    }

    const evidencePage = (response) => {
      if (response?.status !== 'ok') throw new Error(response?.error?.message || '读取失败')
      const lines = Array.isArray(response?.content?.lines) ? response.content.lines.map((line) => ({
        line_number: line.line_number, speaker_raw: line.speaker_raw,
        text: line.text, source_ref: line.source_ref,
      })) : []
      const hasMore = response?.page?.has_more === true
      const rawCursor = response?.page?.next_cursor
      const nextCursor = hasMore && typeof rawCursor === 'string' && rawCursor.length <= 65536
        ? rawCursor : ''
      const total = Number.isInteger(response?.page?.total) && response.page.total >= 0
        ? response.page.total : null
      return { lines, hasMore, nextCursor, total }
    }

    const mergeEvidenceLines = (current, incoming) => {
      const seen = new Set(current.map((line) => `${line.line_number}|${line.source_ref || ''}`))
      return current.concat(incoming.filter((line) => {
        const key = `${line.line_number}|${line.source_ref || ''}`
    ...
    function installDialogFocusTrap(dialog, options = {}) {
      if (!dialog) return () => {}
      const ownerDocument = dialog.ownerDocument
        || (typeof document !== 'undefined' ? document : null)
      if (!ownerDocument) return () => {}
      const focusables = () => [...dialog.querySelectorAll(MODAL_FOCUSABLE_SELECTOR)]
        .filter((element) => element.isConnected !== false
          && element.getAttribute?.('aria-hidden') !== 'true'
          && !element.closest?.('[hidden],[inert],[aria-hidden="true"]')
          && (typeof element.getClientRects !== 'function' || element.getClientRects().length > 0)
          && ownerDocument.defaultView?.getComputedStyle?.(element)?.visibility !== 'hidden')
      const focus = (target) => {
        if (typeof target?.focus !== 'function' || target.isConnected === false) return false
        target.focus()
        return true
      }
      const initial = resolveFocusTarget(options.initialFocus)
      if (!focus(initial)) {
        const first = focusables()[0]
        if (!focus(first)) focus(dialog)
      }
      const onKeyDown = (event) => {
        if (event.defaultPrevented || options.isSuspended?.()) return
        if (event.key === 'Escape') {
        ...
        if (!dialog.contains(ownerDocument.activeElement)) {
          ...
          && (ownerDocument.activeElement === first || ownerDocument.activeElement === dialog)) {
          event.preventDefault()
          focus(last)
      ...
      return () => {
        ...
        focus(resolveFocusTarget(options.returnFocus))
      }
    }

    function SourceReader({ item, onClose, returnFocus }) {
      const [loading, setLoading] = useState(true)
      const [loadingMore, setLoadingMore] = useState(false)
      const [error, setError] = useState('')
      const [lines, setLines] = useState([])
      const [page, setPage] = useState({ hasMore: false, nextCursor: '', total: null })
      ...
      useEffect(() => {
        ...
        setLoading(true); setLoadingMore(false); setError(''); setLines([])
        setPage({ hasMore: false, nextCursor: '', total: null })
        readEvidenceSource(item).then((response) => {
          if (generation.current !== currentGeneration) return
          const first = evidencePage(response)
          setLines(first.lines)
          setPage({ hasMore: first.hasMore, nextCursor: first.nextCursor, total: first.total })
          if (first.hasMore && !first.nextCursor) {
            setError('原文仍有后续内容，但服务器未提供可用的续页定位。')
          }
        }).catch((err) => {
          if (generation.current === currentGeneration) setError(err.message || '读取失败')
        }).finally(() => {
      ...
      useEffect(() => installDialogFocusTrap(sourceDialogRef.current, {
        onEscape: () => onCloseRef.current?.(),
        returnFocus: () => resolveFocusTarget(returnFocusRef.current),
      }), [item.id])
      const loadMore = async () => {
        ...
        try {
          const next = evidencePage(await readEvidenceSource(item, cursor))
          if (generation.current !== currentGeneration) return
          setLines((current) => mergeEvidenceLines(current, next.lines))
          setPage({ hasMore: next.hasMore,
            // 服务器若误返原游标，立即停止，避免用户进入无限续页。
            nextCursor: next.nextCursor === cursor ? '' : next.nextCursor,
            total: next.total ?? page.total })
          if (next.hasMore && (!next.nextCursor || next.nextCursor === cursor)) {
            setError('原文仍有后续内容，但服务器未提供可用的续页定位。')
          }
        } catch (err) {
          if (generation.current === currentGeneration) setError(err.message || '续页读取失败')
        } finally {
      ...
      return h('div', { className: 'prts-evidence-layer' },
        h('button', { type: 'button', className: 'prts-evidence-scrim', onClick: onClose,
          'aria-label': '关闭原文查看器' }),
        h('aside', { ref: sourceDialogRef, className: 'prts-source-reader', role: 'dialog',
        ...
        h('div', { className: 'prts-source-body' },
          ...
          !loading && !error && lines.length === 0 && h('div', { className: 'prts-source-state' },
            '该资料没有可展示的正文。'),
          !loading && lines.length > 0 && h('div', { className: 'prts-source-page',
            role: 'status', 'aria-live': 'polite' }, page.hasMore
            ? `已加载 ${lines.length}${page.total != null ? ` / ${page.total}` : ''} 行，原文尚未完整`
            : `已加载全部 ${lines.length} 行`),
          !loading && lines.map((line) => h('p', {
            className: [
          ...
          line.text)),
          !loading && page.hasMore && page.nextCursor && h('button', { type: 'button',
            className: 'prts-source-more', onClick: loadMore, disabled: loadingMore },
          loadingMore ? '正在加载…' : '继续加载原文'))))
    }
    ...
    function EvidenceControl({ useChat }) {
      ...
      const evidenceDrawerRef = useRef(null)
      const readerReturnFocusRef = useRef(null)
      const readerReturnIdRef = useRef('')
      ...
      const evidenceRevision = useChat ? useChat(evidenceSnapshotSignature) : ''
      ...
      const evidence = useMemo(() => (useChat
        ...
        : domEvidence), [snapshotOrder, snapshotNodes, evidenceRevision, domEvidence])
      useEffect(() => {
        if (!open || reader || typeof document === 'undefined') return undefined
        return installDialogFocusTrap(evidenceDrawerRef.current, {
          onEscape: () => setOpen(false),
          initialFocus: () => readerReturnFocusRef.current,
          returnFocus: () => evidenceTriggerRef.current,
        })
      }, [open, reader])
      return h(Fragment, null,
        h('button', { ref: evidenceTriggerRef, type: 'button', className: 'prts-header-badge',
          onClick: () => {
            readerReturnIdRef.current = ''
            readerReturnFocusRef.current = null
            setOpen(true)
          }, title: '查看本轮本地资料读取记录', 'aria-haspopup': 'dialog',
          'aria-expanded': open },
        ...
        open && !reader && h('div', { className: 'prts-evidence-layer' },
          ...
          h('div', { className: 'prts-evidence-list' }, evidence.length
            ? evidence.map((item, index) => {
                const hasLocator = Boolean(item.sourceRef || item.documentId)
                const readable = hasLocator && validDataVersion(item.dataVersion)
                return h('button', {
                  type: 'button', className: 'prts-evidence-card', key: item.id,
                  ref: (node) => {
                    if (node && readerReturnIdRef.current === item.id) readerReturnFocusRef.current = node
                  },
                  'data-index': String(index + 1).padStart(2, '0'), disabled: !readable,
                  onClick: readable ? (event) => {
                    readerReturnIdRef.current = item.id
                    readerReturnFocusRef.current = event.currentTarget
                    setReader(item)
                  } : undefined,
                  title: readable ? `点击查看原文（${item.documentId || item.sourceRef}）`
                    : hasLocator ? '旧会话未保存完整的 64 位资料版本，不能打开可能已换版的原文'
                      : '旧会话未保存稳定定位，只能查看引用摘要',
                }, h('b', null, readable ? 'CORPUS_READ · VERIFIED' : 'CORPUS_READ · UNVERIFIED'),
    ...
    function SkinCard({ skin, onChanged }) {
      ...
      const choose = async (next) => {
        ...
        const bootToken = changesRuntime ? aicBootShow(normalizedNext === 'endfield-aic'
          ? 'REBUILDING INTERFACE' : 'SWITCHING SKIN', true, normalizedNext) : 0
      ...
      return h('div', { style: S.card },
        ...
        h('div', { className: 'prts-skin-options' },
          ...
          option('rhine-lab', '莱茵生命资料馆', '三维档案阵列、检索过程与可摘录的资料台', 'rhine')),
    ...
    function DatasetLibraryCard({ status, releases, activeRelease, enabledGames, savingGames,
      ...
      return h('div', { style: S.card },
        ...
        h('div', { style: { ...S.hint, marginTop: '9px' } },
          '版本与哈希先由 PRTS.chat current 确认，文件再按镜像顺序下载；切换后索引自动重建。'))
    }
    ...
    /* ---- 设置页主组件 ---- */
    const releaseStatusKey = (status) => JSON.stringify([
      status?.store?.releaseId ?? null, status?.store?.dataVersion ?? null,
      status?.store?.installed ?? null, status?.download?.finishedAt ?? null,
    ])
    ...
    function PrtsSection() {
      ...
      const releaseStatusRef = useRef(null)
      ...
      const refreshStatus = useCallback(async () => {
        ...
        try {
          ...
          assertHostSkinSyncActive(syncGeneration)
          const nextReleaseStatus = releaseStatusKey(nextStatus)
          // Download completion and external activation invalidate the library
          // list. Ordinary status polls keep the last listing without rescanning
          // installed releases; a failed or superseded listing retries next poll.
          if (releaseStatusRef.current !== nextReleaseStatus) {
            const nextReleases = await jsonFetch('/releases')
            if (!aliveRef.current) throw makeAbortError()
            assertHostSkinSyncActive(syncGeneration)
            setReleaseList(nextReleases)
            releaseStatusRef.current = nextReleaseStatus
          }
          setError(null)
        } catch (caught) {
      ...
      const refresh = useCallback(async () => {
        ...
        try {
          ...
          assertHostSkinSyncActive(syncGeneration)
          setReleaseList(nextReleases)
          releaseStatusRef.current = releaseStatusKey(nextStatus)
          setError(null)
        } catch (caught) {
          if (aliveRef.current && syncGeneration === hostSkinSyncGeneration
              && caught?.name !== 'AbortError') setError(caught.message)
      ...
      const download = async () => {
        setBusy(true)
        try {
          await jsonFetch('/download', { method: 'POST', body: JSON.stringify({}) })
      ...
      const statusUpdateInfo = updateInfo?.kind === 'update'
        ? { ...updateInfo, onDownload: download }
        : updateInfo
    ...
    const ensureMapBundle = () => {
      ...
      const pending = new Promise((resolve, reject) => {
        ...
        script.src = `/api/prts-corpus/endfield-map/map.js?abi=${MAP_RUNTIME_ABI}`
        script.async = true
        let settled = false
        let loadTimer = null
        const finish = (callback, value) => {
    ...
    function AicMap({ focusPoint = .66 }) {
      ...
      useEffect(() => {
        ...
        const syncModalState = () => {
          document.body.classList.toggle('aic-modal-open', isHostSettingsOpen())
          syncMapActivity()
        }
    ...
    function AicRoot({ useSessions }) {
      ...
      const openSettings = () => { openHostSettings() }
    ...
    /* Rhine archive: durable returned tool material, independent from camera state. */
    const rhineSkinListeners = new Set()
    const rhineCleanups = new Set()
    let rhineBundlePromise = null
    let rhineBundleOwner = null
    const RHINE_BASE = '/api/prts-corpus/rhine/'
    const rhineNode = (nodes, key) => typeof nodes?.get === 'function' ? nodes.get(key) : nodes?.[key]
    const rhineToolPresentation = (root) => {
      const supported = (meta) => meta?.kind === 'prts-archive-sources-v1' || meta?.kind === 'prts-corpus-read-v1'
      if (supported(root?.meta)) return root.meta
      if (root?.kind !== 'tool-result' || typeof root.parentCallId !== 'string'
          || !Array.isArray(root.content)) return null
      // PTC receipts are their own log-only block, never text to parse or show.
      // Bind all three identities before accepting a durable source projection.
      const receipt = root.content.find((block) => block?.type === 'prts-archive-receipt'
        && block.version === 1 && block.callId === root.callId
        && block.parentCallId === root.parentCallId && block.tool === root.call?.name
        && supported(block.meta) && Array.isArray(block.meta.sources)
        && block.meta.sources.length <= 128)
      return receipt?.meta || null
    }
    const mergeArchiveSources = (sources) => {
      const result = new Map()
      const rank = { found: 0, read: 1, cited: 2 }
      for (const item of sources) {
        if (!item || typeof item.title !== 'string' || !item.title.trim()) continue
        const version = validDataVersion(item.dataVersion) ? item.dataVersion : ''
        const identity = version && (item.documentId || item.documentUid)
          ? `${version}:${item.documentId || item.documentUid}` : item.id
        if (typeof identity !== 'string' || !identity) continue
        const incoming = { ...item, title: item.title.slice(0, 512),
          id: version && item.documentUid ? `document:${version}:${item.documentUid}` : item.id || identity,
          origin: ['cloud', 'web'].includes(item.origin) ? item.origin : 'local',
          state: Object.hasOwn(rank, item.state) ? item.state : 'found',
          excerpt: typeof item.excerpt === 'string' ? item.excerpt : '',
          dataVersion: version || undefined }
        const previous = result.get(identity)
        const ranges = [...(previous?.ranges || []), ...(incoming.ranges || []),
          ...(Number.isInteger(item.lineStart) && item.lineStart > 0
            ? [{ start: item.lineStart, end: item.lineEnd || item.lineStart }] : [])]
          .filter((r) => Number.isSafeInteger(r?.start) && Number.isSafeInteger(r?.end)
            && r.start > 0 && r.end >= r.start).sort((a, b) => a.start - b.start)
        const mergedRanges = []
        for (const range of ranges) {
          const last = mergedRanges.at(-1)
          if (last && range.start <= last.end + 1) last.end = Math.max(last.end, range.end)
          else mergedRanges.push({ start: range.start, end: range.end })
        }
        const readRanges = [...new Map([...(previous?.readRanges || []), ...(incoming.readRanges || [])]
          .filter((r) => Number.isSafeInteger(r?.start) && Number.isSafeInteger(r?.end) && r.start > 0 && r.end >= r.start)
          .map((r) => [`${r.start}:${r.end}`, { start: r.start, end: r.end }])).values()]
        result.set(identity, { ...previous, ...incoming,
          id: version && (incoming.documentUid || previous?.documentUid)
            ? `document:${version}:${incoming.documentUid || previous.documentUid}` : previous?.id || incoming.id,
          origin: previous?.origin === 'cloud' ? 'cloud' : incoming.origin,
          documentId: incoming.documentId || previous?.documentId,
          documentUid: incoming.documentUid || previous?.documentUid,
          sourceRef: incoming.sourceRef || previous?.sourceRef,
          kind: previous?.kind && ['corpus_read', 'reference', 'source'].includes(incoming.kind)
            ? previous.kind : incoming.kind || previous?.kind,
          state: previous && rank[previous.state] > rank[incoming.state] ? previous.state : incoming.state,
          agentRead: previous?.agentRead === true || incoming.agentRead === true
            || previous?.state === 'read' || incoming.state === 'read' || readRanges.length > 0,
          excerpt: incoming.excerpt || previous?.excerpt || '', ranges: mergedRanges,
          readRanges })
      }
      return [...result.values()]
    }
    // A title mention alone is not evidence of citation. Only explicit corpus
    // citation syntax or an exact source_ref can mark a returned source as cited.
    const rhineCitationIndex = sources => {
      const candidates = mergeArchiveSources(sources), titles = new Map(), refs = new Map()
      for (const source of candidates) {
        if (!titles.has(source.title)) titles.set(source.title, [])
        titles.get(source.title).push(source)
        if (source.sourceRef && typeof source.sourceRef === 'string') {
          if (!refs.has(source.sourceRef)) refs.set(source.sourceRef, [])
          refs.get(source.sourceRef).push(source)
        }
      }
      const exactRefs = []
      for (const [ref, hits] of refs) {
        ...
        exactRefs.push([hits[0].id, new RegExp(`(?:^|[\\s\\x60("'=:])${escaped}(?=$|[\\s\\x60)"'\\],.;，。；])`, 'u')])
      }
      return { titles, exactRefs }
    }
    const citedArchiveIds = (sources, answer, wikiCitations, index = rhineCitationIndex(sources)) => {
      const cited = new Set()
      const matchesRange = (source, start, end) => (source.ranges || []).some((range) =>
        range.start <= start && range.end >= end)
      for (const match of answer.matchAll(/《([^》\n]+)》(?:（document_uid=(doc_[A-Za-z0-9_-]{16})）)?\s*(?:第\s*(\d+)(?:\s*[-—–~]\s*(\d+))?\s*行|Wiki·([^\s。；，,;\n]+))/gu)) {
        const start = Number(match[3]), end = Number(match[4] || match[3])
        const hits = (index.titles.get(match[1]) || []).filter(source => !match[2] || source.documentUid === match[2])
        // UID alone cannot disambiguate two revisions of the same document.
        if (hits.length !== 1) continue
        const source = hits[0]
        if (match[5] ? wikiCitations?.get(source.id)?.has(match[5])
          : Number.isSafeInteger(start) && start > 0 && end >= start && matchesRange(source, start, end)) cited.add(source.id)
      }
      for (const [id, pattern] of index.exactRefs) if (pattern.test(answer)) cited.add(id)
      return cited
    }
    const isRhineWebTool = name => name === 'web_search' || name === 'web_fetch'
    const isRhineReadTool = name => name === 'corpus_read' || name === 'cloud_inspect' || name === 'web_fetch'
    const rhineWebUrl = value => {
    ...
    // Consume the web tools' durable structured metadata, never links guessed
    // from arbitrary tool output. Fetch bodies remain the exact returned text.
    const rhineWebSources = (name, args, meta, text, callId, prior) => {
      if (name === 'web_search') return (Array.isArray(meta?.sources) ? meta.sources : []).slice(0, 128).flatMap(item => {
        const url = rhineWebUrl(item?.url)
        if (!url) return []
        return [{ id: `web:${url}`, url, title: typeof item.title === 'string' && item.title.trim() ? item.title : url,
          kind: 'web_page', origin: 'web', state: 'found', callId,
          excerpt: typeof item.snippet === 'string' ? item.snippet : '' }]
      })
      const requested = rhineWebUrl(args.url), url = rhineWebUrl(meta?.url) || requested
      if (!url || !text.trim() || Number(meta?.statusCode) >= 400) return []
      const id = `web:${requested || url}`
      const previous = [...prior].reverse().find(source => source.id === id || source.origin === 'web' && rhineWebUrl(source.url) === url)
      const heading = text.match(/^#{1,2}\s+([^\n]+)$/mu)?.[1]?.trim()
      return [{ id: previous?.id || id, url, title: previous?.title || heading || url,
        kind: 'web_page', origin: 'web', state: 'read', agentRead: true, callId,
        excerpt: text.slice(0, 1600), content: text, contentTruncated: meta?.truncated === true }]
    }
    const rhinePhase = (snapshot) => {
      if (snapshot.searching) return isRhineReadTool(snapshot.tool) ? 'reading' : 'searching'
      if (snapshot.running) return 'synthesizing'
      if (snapshot.error) return 'error'
      return snapshot.answer ? 'complete' : 'idle'
    }
    // Detached input values detect same-length and in-place edits. Equal
    // subtrees stay shared without serializing the entire transcript per tick.
    const retainRhineValue = (value, previous) => {
      if (Object.is(value, previous)) return previous
      if (!value || typeof value !== 'object') return value
      const array = Array.isArray(value)
      const compatible = previous && typeof previous === 'object' && Array.isArray(previous) === array
      const keys = Object.keys(value)
      const previousKeys = compatible ? Object.keys(previous) : []
      let result = compatible ? undefined : array ? [] : {}
      const copy = () => result ||= array ? previous.slice() : { ...previous }
      for (const key of keys) {
        const next = retainRhineValue(value[key], compatible ? previous[key] : undefined)
        if (!compatible || !Object.hasOwn(previous, key) || !Object.is(next, previous[key])) {
          copy()
          if (key === '__proto__') Object.defineProperty(result, key, { value: next, enumerable: true, writable: true, configurable: true })
          else result[key] = next
        }
      }
      for (const key of previousKeys) if (!Object.hasOwn(value, key)) { copy(); delete result[key] }
      if (array && (!compatible || value.length !== previous.length)) { copy(); result.length = value.length }
      return result || previous
    }
    const rhineSnapshotEntries = (order, nodes) => {
      const entries = []
      for (const key of order || []) {
        const node = rhineNode(nodes, key)
        entries.push([key, node])
        if (node?.kind !== 'tool-call') continue
        const seen = new Set([node.data?.root])
        const queue = [...(node.data?.root?.subCalls || [])].reverse()
        let count = 0
        while (queue.length && count++ < 16384) {
          const child = queue.pop()
          if (!child || seen.has(child)) continue
          seen.add(child)
          entries.push([`${key}:${child.callId || count}`, { kind: 'tool-call', data: { root: child },
            location: node.location, anchorSeq: node.anchorSeq }])
          queue.push(...(child.subCalls || []).slice().reverse())
        }
      }
      return entries
    }
    const rhineTurnInput = turn => turn ? { turn: turn.turn, start: turn.start, end: turn.end } : undefined
    const rhineSnapshotInput = snapshot => {
      const entries = rhineSnapshotEntries(snapshot?.order, snapshot?.nodes).map(([key, node]) => {
        if (!node) return [key, node]
        const raw = node.data, root = raw?.root
        const data = { seq: raw?.seq, turn: raw?.turn }
        if (node.kind === 'assistant-step') {
          data.status = raw?.status
          data.blocks = [{ kind: 'text', text: visibleAssistantText(raw?.blocks) }]
        } else if (node.kind === 'user' || node.kind === 'steering') {
          data.content = (raw?.content || []).filter(block => block?.type === 'text')
            .map(block => ({ type: 'text', text: block.text }))
        } else if (node.kind === 'tool-call' && root) {
          data.root = { name: root.name, argsRaw: root.argsRaw,
            ...('kind' in root ? { kind: root.kind } : {}),
            call: root.call ? { name: root.call.name, argsRaw: root.call.argsRaw } : undefined,
            meta: root.meta, content: root.content, isError: root.isError, time: root.time,
            callTime: root.callTime, callId: root.callId, parentCallId: root.parentCallId,
            turn: root.turn, seq: root.seq }
        } else if (node.kind === 'turn-error' || node.kind === 'turn-max-tokens') data.message = raw?.message
        return [key, { kind: node.kind, data, anchorSeq: node.anchorSeq,
          location: { turn: rhineTurnInput(node.location?.turn) } }]
      })
      const turnNumber = snapshot?.timeline?.turnOrder?.at(-1)
      const turn = turnNumber === undefined ? undefined : snapshot?.timeline?.turns?.get(turnNumber)
      return { entries, turnNumber, turn: rhineTurnInput(turn) }
    }
    function createRhineProjectionCache(diagnostics) {
      const slots = new Map()
      let used = new Set()
      return {
        begin() { used = new Set() },
        end() { for (const key of slots.keys()) if (!used.has(key)) slots.delete(key) },
        array(key, value) {
          used.add(key)
          const previous = slots.get(key)?.value
          if (previous?.length === value.length && value.every((item, i) => Object.is(item, previous[i]))) return previous
          const next = value.slice()
          slots.set(key, { value: next })
          return next
        },
        retain(key, value) {
          used.add(key)
          const next = retainRhineValue(value, slots.get(key)?.value)
          slots.set(key, { value: next })
          return next
        },
        memo(key, deps, task, category) {
          used.add(key)
          const previous = slots.get(key)
          if (previous?.deps?.length === deps.length && deps.every((value, i) => Object.is(value, previous.deps[i]))) {
            diagnostics?.cacheEvent(category, true)
            return previous.value
          }
          diagnostics?.cacheEvent(category, false)
          const value = task()
          slots.set(key, { deps, value })
          return value
        },
      }
    }
    const buildRhineSnapshot = (order = [], nodes, sessionId = '', sessionRunning = false, timeline, cache) => {
      const memo = (key, deps, task, category) => cache ? cache.memo(key, deps, task, category) : task()
      const retain = (key, value) => cache ? cache.retain(key, value) : value
      const retainArray = (key, value) => cache ? cache.array(key, value) : value
      const sources = []
      const records = []
      const operations = []
      const toolCalls = []
      const operationSources = new Map()
      const entries = cache?.entries || rhineSnapshotEntries(order, nodes)
      const userEntries = entries.filter(([, node]) => node?.kind === 'user' || node?.kind === 'steering')
      const lastInput = userEntries.at(-1)
      const inputSeq = lastInput?.[1]?.data?.seq ?? lastInput?.[1]?.anchorSeq
      const turnNumber = timeline?.turnOrder?.at(-1)
      let currentTurn = turnNumber === undefined ? undefined : timeline?.turns?.get(turnNumber)
      if (!currentTurn) {
        currentTurn = [...entries].reverse().find(([, node]) => node?.location?.turn)?.[1]?.location?.turn
      }
      // A newly admitted message can precede its turn/start publication. Never
      // reuse the previous turn's successful end for that pending question.
      if (currentTurn?.end && Number.isFinite(inputSeq) && inputSeq > currentTurn.end.seq) currentTurn = undefined
      const investigationId = currentTurn ? `turn:${currentTurn.turn}`
        : lastInput ? `message:${lastInput[0]}` : entries.length ? `node:${entries.at(-1)[0]}` : ''
      const belongsToCurrentTurn = (node) => {
        if (!currentTurn) return true
        const owner = node?.location?.turn?.turn ?? node?.data?.turn ?? node?.data?.root?.turn
        if (owner !== undefined) return owner === currentTurn.turn
        const seq = node?.anchorSeq ?? node?.data?.seq ?? node?.data?.root?.seq
        return Number.isFinite(seq) && seq >= (currentTurn.start?.seq ?? Infinity)
          && seq <= (currentTurn.end?.seq ?? Infinity)
      }
      let turnSourceStart = 0
      const turnSources = []
      let searching = false
      let question = ''
      let query = ''
      ...
      let activeArgs = null
      const cited = new Set()
      const wikiCitations = new Map()
      const toolErrors = new Map()
      const citations = (key, material, text) => {
        if (!cache) return citedArchiveIds(material, text, wikiCitations)
        const input = retainArray('citation-input:' + key, material)
        const wiki = retain('wiki-input:' + key, [...wikiCitations].map(([id, values]) => [id, [...values]]))
        const candidates = memo('citation-index:' + key, [input], () => rhineCitationIndex(input), 'citation')
        return memo('citations:' + key, [candidates, wiki, text], () => citedArchiveIds(input, text, wikiCitations, candidates), 'citation')
      }
      for (const [key, node] of entries) {
        if (node?.kind === 'user' || node?.kind === 'steering') {
          const text = (node.data?.content || []).filter((block) => block?.type === 'text')
            .map((block) => block.text || '').join('\n').trim()
          question = text
          // New user input starts a new answer; keep the durable material rack.
          answer = ''; error = ''; assistantRunning = false; toolErrors.clear()
          interrupted = false; terminalError = false
          // With a real Turn location, steering belongs to the same ongoing
          // investigation. Older renderers fall back to the last input boundary.
          if (!currentTurn) { turnSourceStart = sources.length; operations.length = 0; toolCalls.length = 0; operationSources.clear() }
          if (node.kind === 'user' || !currentTurn) { searching = false; activeArgs = null }
          if (!searching) { query = ''; tool = '' }
          continue
        }
        if (node?.kind === 'assistant-step') {
          const text = visibleAssistantText(node.data?.blocks)
          const current = belongsToCurrentTurn(node)
          if (current) assistantRunning = node.data?.status === 'running'
          if (text) {
            if (current) answer = text
            for (const id of citations('assistant:' + key, sources, text)) cited.add(id)
          }
          if (current && node.data?.status === 'interrupted') { error = '调查已停止'; interrupted = true }
          continue
        }
        if (node?.kind === 'turn-error' || node?.kind === 'turn-max-tokens') {
          if (!belongsToCurrentTurn(node)) continue
          error = node.data?.message || (node.kind === 'turn-max-tokens' ? '回答达到输出上限，调查未完成' : '调查未完成')
          terminalError = true
        ...
        if (node?.kind !== 'tool-call') continue
        const root = node.data?.root
        const projection = memo('tool:' + key, [root], () => {
          const call = toolSnapshot(root)
          const meta = rhineToolPresentation(root)
          if ((typeof meta?.error === 'string' && meta.error || call?.name === 'web_fetch' && Number(root?.meta?.statusCode) >= 400) && call) call.state = 'error'
          return { call, meta, args: parseToolArguments(call?.argsRaw),
            text: root?.kind === 'tool-result' ? contentText(root.content) : '' }
        }, 'tool')
        const { call, meta, args, text } = projection
        if (!call?.name) continue
        const locator = args.locator && typeof args.locator === 'object' ? args.locator : args
        const currentQuery = String(args.query || args.title || locator.display_title || locator.document_id
          || locator.document_uid || locator.source_ref || args.url
          || (Array.isArray(args.queries) ? args.queries.filter(item => typeof item === 'string').join(' · ') : '')
          || args.activity_name || args.collection_name || args.section || '')
        const current = belongsToCurrentTurn(node)
        records.push(memo('record:' + key, [call, currentQuery, text], () =>
          ({ id: String(key), tool: call.name, state: call.state, query: currentQuery, text }), 'tool'))
        if (current) {
          const startedAt = root.kind === 'tool-result' ? root.callTime : root.time
          const completedAt = root.kind === 'tool-result' ? root.time : undefined
          toolCalls.push({ id: String(key), tool: call.name, state: call.state,
            query: currentQuery.slice(0, 240),
            ...(Number.isFinite(startedAt) ? { startedAt } : {}),
            ...(Number.isFinite(completedAt) ? { completedAt } : {}) })
        }
        // Corpus and web material both enter the archive; unrelated tools remain in the HUD.
        if (!PRTS_SCENE_TOOLS.includes(call.name) && !isRhineWebTool(call.name)) continue
        if (current) {
          const requestedVersion = args.data_version || args.dataVersion || locator.data_version || locator.dataVersion
          const startedAt = root.kind === 'tool-result' ? root.callTime : root.time
          const completedAt = root.kind === 'tool-result' ? root.time : undefined
          operations.push({ id: String(key), tool: call.name, kind: isRhineReadTool(call.name) ? 'read' : 'search',
            state: call.state, sourceIds: [], ...(currentQuery ? { query: currentQuery } : {}),
            ...(call.name === 'web_fetch' && rhineWebUrl(args.url) ? { url: rhineWebUrl(args.url) } : {}),
            ...(typeof locator.document_id === 'string' && locator.document_id ? { documentId: locator.document_id } : {}),
            ...(typeof locator.document_uid === 'string' && locator.document_uid ? { documentUid: locator.document_uid } : {}),
            ...(typeof locator.source_ref === 'string' && locator.source_ref ? { sourceRef: locator.source_ref } : {}),
            ...(validDataVersion(requestedVersion) ? { dataVersion: requestedVersion } : {}),
            ...(Number.isFinite(startedAt) ? { startedAt } : {}),
            ...(Number.isFinite(completedAt) ? { completedAt } : {}) })
        }
        if (current && call.state === 'active' && !node.location?.turn?.end) {
          searching = true
          // PTC can run reads and searches together. Keep the reading operation
          // as the visual focus instead of letting a later search in tree order
          // hide its cassette animation or overwrite its document locator.
          if (!activeArgs || isRhineReadTool(call.name) || !isRhineReadTool(tool)) {
            query = currentQuery
            tool = call.name
            activeArgs = args
          }
        } else if (current && !searching) { query = currentQuery || query; tool = call.name }
        if (call.state === 'error') {
          if (current) toolErrors.set(call.name, meta?.error || text.slice(0, 600) || `${call.name} 执行失败`)
          continue
        }
        if (call.state !== 'complete') continue
        if (current) toolErrors.delete(call.name)
        const sourceStart = sources.length
        if (isRhineWebTool(call.name)) {
          const prior = call.name === 'web_fetch' ? retainArray('web-prior:' + key, sources.filter(source => source.origin === 'web')) : undefined
          sources.push(...memo('web-sources:' + key, [call.name, args, root.meta, text, prior],
            () => rhineWebSources(call.name, args, root.meta, text, String(key), prior || []), 'material'))
        }
        if (meta?.kind === 'prts-archive-sources-v1' && Array.isArray(meta.sources)) {
          sources.push(...memo('delivered:' + key, [meta.sources], () => meta.sources.slice(0, 128)
            .map(source => ({ ...source, callId: String(key) })), 'material'))
        }
        if (call.name === 'corpus_read') {
          const delivered = memo('read-sources:' + key, [meta, text], () => {
            const read = readPresentation(meta)
            const readSources = read ? read.sources.length ? read.sources : [read] : []
            const material = [], wiki = []
            for (const source of readSources) {
              const id = `document:${source.dataVersion}:${source.documentUid || source.documentId || source.sourceRef}`
              for (const citation of text.matchAll(/^引用：《([^》\n]+)》Wiki·([^\s\n]+)$/gmu)) {
                if (citation[1] === (source.title || read.title)) wiki.push([id, citation[2]])
              }
              material.push({ ...source, id,
                title: source.title || read.title || '原文资料', kind: 'corpus_read',
                origin: 'local', state: 'read', callId: String(key),
                sourceRef: source.sourceRef || (read.sources.length <= 1 ? read.sourceRef : ''),
                readRanges: source.lineStart && source.lineEnd ? [{ start: source.lineStart, end: source.lineEnd }] : [],
                excerpt: read.sources.length > 1
                  ? `Agent 已读取《${source.title}》第 ${source.lineStart}–${source.lineEnd} 行；完整返回内容见检索记录。`
                  : text.slice(0, 1600) })
            }
            return { material, wiki }
          }, 'material')
          sources.push(...delivered.material)
          for (const [id, field] of delivered.wiki) {
            if (!wikiCitations.has(id)) wikiCitations.set(id, new Set())
            wikiCitations.get(id).add(field)
          }
        }
        if (current) {
          const delivered = sources.slice(sourceStart)
          operationSources.set(String(key), delivered)
          if (currentTurn) turnSources.push(...delivered)
        }
      }
      const sourceInput = retainArray('source-input', sources)
      const mergedMaterials = memo('merge-materials', [sourceInput], () => mergeArchiveSources(sourceInput), 'material')
      const citedInput = retain('cited-input', [...cited])
      const materials = memo('materials', [mergedMaterials, citedInput], () => mergedMaterials.map(source => cited.has(source.id)
        ? { ...source, state: 'cited' } : source), 'material')
      error = error || [...toolErrors.values()].at(-1) || ''
      const end = currentTurn?.end
      const endKind = end?.type === 'turn/end' ? end.data?.reason?.kind : undefined
      if (interrupted || terminalError) { searching = false; assistantRunning = false }
      let outcome = terminalError ? 'error' : interrupted ? 'interrupted'
        : sessionRunning || searching || assistantRunning ? 'running' : 'unknown'
      if (endKind) {
        // A finished turn closes its tools too. A stale streamed node must not
        // keep animating a cancelled or completed investigation after its end.
        searching = false; assistantRunning = false
        if (endKind === 'completed') { outcome = 'completed'; error = '' }
        else if (endKind === 'aborted' || endKind === 'interrupted') { outcome = 'interrupted'; error = '调查已停止' }
        else if (endKind === 'error' || endKind === 'max-tokens' || endKind === 'blocked') {
          outcome = 'error'
          error = end.data.reason.error?.message || (endKind === 'max-tokens'
            ? '回答达到输出上限，调查未完成' : endKind === 'blocked' ? '调查被阻止，尚未完成' : error || '调查未完成')
        } else { outcome = 'unknown' }
      }
      // Keep one operation per invocation instead of inferring activity from a
      // source's highest-ever state. A second read of the same ranges is still
      // a new operation, even though it makes no change to the material rack.
      const sourceAliases = (source, includeRef = true) => {
        const version = validDataVersion(source.dataVersion) ? source.dataVersion : ''
        return [['id', source.id], ['uid', source.documentUid], ['document', source.documentId],
          ...(includeRef ? [['ref', source.sourceRef]] : [])]
          .flatMap(([kind, value]) => value ? [JSON.stringify([version, kind, value])] : [])
      }
      const materialIds = memo('material-aliases', [mergedMaterials], () => {
        const result = new Map()
        for (const source of mergedMaterials) {
          for (const alias of sourceAliases(source)) {
            if (!result.has(alias)) result.set(alias, new Set())
            result.get(alias).add(source.id)
          }
        }
        return result
      }, 'material')
      const deliveredMaterialId = (source) => {
        // A shared source_ref can cover several documents. Prefer the first
        // matching exact identity, never union a weaker alias into a receipt.
        const precise = Boolean(source.documentUid || source.documentId)
        for (const alias of sourceAliases(source, !precise)) {
          const matches = materialIds.get(alias)
          if (matches?.size) return matches.size === 1 ? [...matches] : []
        }
        return []
      }
      for (const call of toolCalls) {
        if (call.state === 'active' && (endKind || interrupted || terminalError)) {
          call.state = 'error'
          const index = records.findIndex(record => record.id === call.id)
          if (index >= 0) records[index] = { ...records[index], state: 'error' }
        }
      }
      for (const operation of operations) {
        if (operation.state === 'active' && (endKind || interrupted || terminalError)) operation.state = 'error'
        const delivered = (operationSources.get(operation.id) || []).filter(source => operation.kind !== 'read'
          || source.agentRead === true || source.state === 'read' || source.readRanges?.length > 0)
        operation.sourceIds = [...new Set(delivered.flatMap(deliveredMaterialId))]
      }
      // Report citations are recomputed from this answer only. Historical
      // citations still mark the cumulative rack, but never enter a new report.
      const reportIds = citations('report', materials, answer)
      const reportInput = retainArray('report-ids', [...reportIds])
      const reportSources = memo('report-sources', [materials, reportInput], () => materials.filter(source => reportIds.has(source.id))
        .map(source => ({ ...source, state: 'cited' })), 'material')
      const turnInput = retainArray('turn-input', currentTurn ? turnSources : sources.slice(turnSourceStart))
      const mergedTurn = memo('merge-turn', [turnInput], () => mergeArchiveSources(turnInput), 'material')
      const currentSources = memo('current-sources', [mergedTurn, reportInput], () => mergedTurn
        .map(source => reportIds.has(source.id) ? { ...source, state: 'cited' } : source), 'material')
      const activeLocator = activeArgs?.locator || activeArgs || {}
      let activeDocumentId = typeof activeLocator.document_id === 'string' ? activeLocator.document_id : ''
      const activeSourceRef = typeof activeLocator.source_ref === 'string' ? activeLocator.source_ref : ''
      const requestedVersion = activeArgs?.data_version || activeArgs?.dataVersion
        || activeLocator.data_version || activeLocator.dataVersion
      const activeDataVersion = validDataVersion(requestedVersion) ? requestedVersion : ''
      if (!activeDocumentId && isRhineReadTool(tool)) {
        const hits = materials.filter((source) => activeLocator.document_uid
          ? source.documentUid === activeLocator.document_uid
          : activeSourceRef ? source.sourceRef === activeSourceRef
            : (activeLocator.title || activeLocator.display_title)
              && source.title === (activeLocator.title || activeLocator.display_title))
          .filter((source) => !activeDataVersion || source.dataVersion === activeDataVersion)
        if (hits.length === 1) activeDocumentId = hits[0].documentId || ''
      }
      const snapshot = { sessionId, running: !endKind && !interrupted && !terminalError && Boolean(sessionRunning || searching || assistantRunning || toolCalls.some(call => call.state === 'active')), searching, question, query, tool,
        investigationId, outcome, turnSources: currentSources, reportSources,
        ...(outcome === 'completed' && Number.isFinite(end?.time) ? { completedAt: end.time } : {}),
        sources: materials, answer, records, operations, toolCalls, ...(error ? { error } : {}),
        ...(searching && activeDocumentId ? { activeDocumentId } : {}),
        ...(searching && activeSourceRef ? { activeSourceRef } : {}),
        ...(searching && activeDataVersion ? { activeDataVersion } : {}) }
      return retain('output', { ...snapshot, phase: rhinePhase(snapshot) })
    }
    // Each mounted controller owns bounded counters; no snapshot data is exported.
    function createRhineSnapshotDiagnostics() {
      let monitor
      const counts = Object.fromEntries(['host-snapshot-selector', 'host-snapshot-build',
        'host-snapshot-serialize', 'host-snapshot-parse', 'host-snapshot-merge',
        'host-snapshot-derive', 'host-snapshot-input'].map(kind => [kind, 0]))
      const cache = Object.fromEntries(['input', 'tool', 'material', 'citation', 'serialize', 'sourceMerge']
        .flatMap(kind => [[kind + 'Hits', 0], [kind + 'Misses', 0]]))
      return {
        cacheEvent(kind, hit) {
          const key = kind + (hit ? 'Hits' : 'Misses')
          if (Object.hasOwn(cache, key)) cache[key]++
        },
        measure(kind, task) {
          if (!Object.prototype.hasOwnProperty.call(counts, kind)) return task()
          counts[kind]++
          const done = monitor?.(kind)
          try { return task() } finally { done?.() }
        },
        setPerformanceMonitor(value) { monitor = value },
        performanceStats() { return { counts: { ...counts }, cache: { ...cache } } },
      }
    }
    const rhineSnapshotSignature = (snapshot, diagnostics) => {
      const measure = (kind, task) => diagnostics ? diagnostics.measure(kind, task) : task()
      return measure('host-snapshot-selector', () => {
        const value = measure('host-snapshot-build', () => buildRhineSnapshot(snapshot?.order, snapshot?.nodes, '', false, snapshot?.timeline))
        return measure('host-snapshot-serialize', () => JSON.stringify(value))
      })
    }

    // Preserve the host's string selector contract, but serialize only changed
    // fields and let React consume the already-built detached value. A stale
    // concurrent render can still parse its own string, never a newer snapshot.
    function createRhineSnapshotSelector(diagnostics) {
      const measure = (kind, task) => diagnostics ? diagnostics.measure(kind, task) : task()
      const cache = createRhineProjectionCache(diagnostics)
      const fields = new Map()
      let input, value, serialized
      const select = snapshot => measure('host-snapshot-selector', () => {
        const nextInput = measure('host-snapshot-input', () => retainRhineValue(rhineSnapshotInput(snapshot), input))
        if (nextInput === input) {
          diagnostics?.cacheEvent('input', true)
          return serialized
        }
        diagnostics?.cacheEvent('input', false)
        cache.begin()
        cache.entries = nextInput.entries
        let next
        try {
          const timeline = { turnOrder: [nextInput.turnNumber], turns: new Map([[nextInput.turnNumber, nextInput.turn]]) }
          next = measure('host-snapshot-build', () => buildRhineSnapshot([], null, '', false, timeline, cache))
        } finally { cache.end(); cache.entries = undefined }
        const signature = measure('host-snapshot-serialize', () => {
          const parts = [], keys = new Set(Object.keys(next))
          for (const key of fields.keys()) if (!keys.has(key)) fields.delete(key)
          for (const [key, item] of Object.entries(next)) {
            let field = fields.get(key)
            const hit = Boolean(field && Object.is(field.value, item))
            diagnostics?.cacheEvent('serialize', hit)
            if (!hit) { field = { value: item, text: JSON.stringify(item) }; fields.set(key, field) }
            if (field.text !== undefined) parts.push(JSON.stringify(key) + ':' + field.text)
          }
          return '{' + parts.join(',') + '}'
        })
        input = nextInput; value = next; serialized = signature
        return serialized
      })
      select.read = signature => signature === serialized && value ? value
        : measure('host-snapshot-parse', () => signature ? JSON.parse(signature) : buildRhineSnapshot([], null))
      return select
    }

    // Both resources are staged together and retired on HMR. Failed loads are retryable.
    const loadRhineBundle = () => {
      if (rhineBundlePromise) return rhineBundlePromise
      const owner = { elements: [], cancel: null }
      rhineBundleOwner = owner
      const load = (kind, path) => new Promise((resolve, reject) => {
        const element = document.createElement(kind === 'script' ? 'script' : 'link')
        if (kind === 'script') { element.src = RHINE_BASE + path; element.async = true }
        else { element.rel = 'stylesheet'; element.href = RHINE_BASE + path }
        element.dataset.plugin = 'prts-terrarchive'
        element.dataset.rhineResource = path
        owner.elements.push(element)
        const abort = () => finish(makeAbortError('资料馆资源加载已取消'))
        const timer = setTimeout(() => finish(new Error('资料馆资源加载超时，请重试')), 15000)
        const signal = clientAbortController?.signal
        const finish = (error) => {
          clearTimeout(timer)
        ...
        element.onerror = () => finish(new Error(`资料馆资源加载失败：${path}`))
        if (signal?.aborted) abort()
        else document.head.appendChild(element)
      })
      rhineBundlePromise = Promise.all([load('style', 'rhine.css'), load('script', 'rhine.js')])
        .then(() => {
          const runtime = globalThis.__PRTS_RHINE__
          if (rhineBundleOwner !== owner) throw makeAbortError('资料馆已被新实例替换')
          if (runtime?.RUNTIME_ABI !== 1 || typeof runtime.mountRhineWorkbench !== 'function') {
            throw new Error('资料馆资源版本不兼容，请刷新页面')
          }
          return runtime
        }).catch((error) => {
          for (const element of owner.elements) element.remove()
          if (rhineBundleOwner === owner) { rhineBundleOwner = null; rhineBundlePromise = null }
          throw error
        })
      return rhineBundlePromise
    }
    const disposeRhine = () => {
      for (const cleanup of [...rhineCleanups]) cleanup()
      for (const element of rhineBundleOwner?.elements || []) element.remove()
      rhineBundleOwner = null
      rhineBundlePromise = null
    }
    async function submitRhineQuestion(ctx, sessionId, text, signal) {
      if (!text?.trim()) return
      const session = ctx?.sessions?.binding?.(sessionId)?.session
      if (!session?.prompt || !session?.beginSubmission) throw new Error('当前会话尚未连接，请返回对话后重试')
      ...
      try {
        ...
        if (!result?.ok) throw new Error(result?.error?.message || 'Agent 未接受问题，请重试')
      } catch (error) { submission.abandon(); throw error }
    }
    // The workbench uses the same public session and model services as DSH's
    // sidebar/composer. Each controller is owned by one open archive session.
    function createRhineHostControls(ctx, sessionId) {
      ...
      const metrics = { notifications: 0, coalesced: 0, flushes: 0, stateReads: 0, emitted: 0, deduplicated: 0,
        sources: { sessions: 0, workspaces: 0, session: 0, models: 0 } }
      const cancelPending = () => {
        if (frame !== null) window.cancelAnimationFrame(frame)
        if (timer !== null) clearTimeout(timer)
        frame = timer = null
      }
      const modelKey = (selection) => selection ? JSON.stringify([selection.provider, selection.model]) : ''
      const workspaceState = () => ctx.workspaces?.list.getSnapshot() || { items: [], archivedSessionIds: [] }
      const workspaceOf = (id) => workspaceState().items.find(item => item.sessionIds?.includes(id))
      const summaryOf = () => sessions.list.getSnapshot().byId[sessionId]
      const modelOptions = () => (directory?.store.getSnapshot().groups || []).flatMap(group =>
        (group.models || []).map(model => ({
          id: modelKey({ provider: group.id, model: model.id }),
          name: `${model.name || model.id} · ${group.name || group.id}`,
          provider: group.id, model: model.id, reasoning: model.reasoning,
        })))
      const projectState = () => {
        ...
        const modelState = directory?.store.getSnapshot()
        const currentModel = modelState?.current
        const choices = modelOptions().map(({ id, name }) => ({ id, name }))
        // Catalog membership is advisory; an existing routable model may no
        // longer be advertised. Preserve its actual selection in the picker.
        if (currentModel && !choices.some(item => item.id === modelKey(currentModel))) {
          choices.unshift({ id: modelKey(currentModel), name: `${currentModel.model} · ${currentModel.provider}` })
        }
        const workspace = workspaceOf(sessionId)
        return {
          sessionId, title: current?.title || (current?.blank ? '新建对话' : current?.displayTitle) || '当前对话',
          workspace: workspace?.title || workspace?.path || current?.cwd || '',
          blank: Boolean(lifecycle?.blank ?? current?.blank),
          running: Boolean(lifecycle?.running ?? current?.running),
          busy: busy || modelState?.status === 'selecting', loading,
          canSubmit: modelState?.routable !== false && !lifecycle?.removed,
          error: error || (modelState?.routable === false ? '当前模型没有可用连接，请选择其他模型。' : modelState?.error || ''),
          mode: typeof current?.projectionValues?.agentPreset === 'string' ? current.projectionValues.agentPreset : '',
          modes, model: modelKey(currentModel), models: choices,
          sessions: list.ids.map(id => list.byId[id]).filter(item => item && !archived.has(item.id) && item.origin !== 'subagent')
            .map(item => {
              const owner = workspaces.items.find(workspace => workspace.sessionIds?.includes(item.id))
              return { id: item.id, title: item.title || (item.blank ? '新建对话' : item.displayTitle) || '未命名对话',
                workspace: owner?.title || owner?.path || item.cwd || '',
                running: Boolean(item.running), updatedAt: item.updatedAt || 0 }
            }),
        }
      }
      const getState = () => {
        metrics.stateReads++
        const done = monitor?.('host-state-project')
        try { return projectState() } finally { done?.() }
      }
      // The history UI displays a date, not per-token timestamp changes. Preserve
      // ordering, names and all actionable fields; never compare only references.
      const displaySignature = state => JSON.stringify({ ...state, sessions: state.sessions.map(item => ({
        ...item, updatedAt: Number.isFinite(item.updatedAt) && item.updatedAt > 0 ? new Date(item.updatedAt).toLocaleDateString('zh-CN') : '',
      })) })
      let publishedSignature = displaySignature(getState())
      const emit = () => {
        cancelPending()
        if (disposed) return
        metrics.flushes++
        const done = monitor?.('host-notify')
        try {
          const state = getState(), signature = displaySignature(state)
          if (signature === publishedSignature) { metrics.deduplicated++; return }
          publishedSignature = signature; metrics.emitted++
          for (const listener of listeners) listener(state)
        } finally { done?.() }
      }
      const requestEmit = source => {
        if (disposed) return
        metrics.notifications++; metrics.sources[source]++
        if (!listeners.size) return
        if (frame !== null || timer !== null) { metrics.coalesced++; return }
        if (typeof window.requestAnimationFrame === 'function') frame = window.requestAnimationFrame(emit)
        // RAF may be suspended in a background tab; current state still arrives.
        timer = setTimeout(emit, frame === null ? 16 : 100)
      }
      const assertActive = () => {
        if (disposed || clientContext !== ctx || sessions.list.getSnapshot().current !== sessionId) {
          throw makeAbortError('会话已切换，请在当前会话重试')
        }
      }
      const unwrap = (result, label) => {
        if (!result?.ok) throw new Error(result?.error?.message || `${label}失败`)
        return result.value
      }
      const run = async (action) => {
        assertActive()
        if (busy || directory?.store.getSnapshot().status === 'selecting') throw new Error('上一项设置正在保存，请稍候')
        busy = true; error = ''; emit()
        try { await action() }
        catch (caught) {
          if (!disposed && caught?.name !== 'AbortError') error = caught?.message || '操作失败，请重试'
          throw caught
      ...
      const refresh = () => {
        ...
        refreshTask = (async () => {
          const results = await Promise.allSettled([
            waitWithDeadline(ctx.remote.agentPresets.list(), { timeout: 12000, label: '模式列表' }),
            waitWithDeadline(sessions.refresh(), { timeout: 12000, label: '会话列表' }),
            directory ? waitWithDeadline(directory.load(), { timeout: 12000, label: '模型列表' }) : Promise.resolve(),
          ])
          if (disposed) return
          const roster = results[0]
          if (roster.status === 'fulfilled') {
            try {
              const value = unwrap(roster.value, '读取模式')
              modes = (value.presets || []).filter(item => !item.broken)
                .map(item => ({ id: item.id, name: item.name || item.id, description: item.description || '' }))
            } catch (caught) { error = caught.message }
          }
          const failures = results.filter(result => result.status === 'rejected')
            .map(result => result.reason?.message || '读取会话设置失败')
          if (failures.length) error = [error, ...failures].filter(Boolean).join('；')
        })().finally(() => { loading = false; refreshTask = null; emit() })
        return refreshTask
      }
      const requireMode = (id) => {
        if (!id || !modes.some(mode => mode.id === id)) throw new Error('该模式暂不可用，请刷新模式列表')
      }
      const createSession = (requestedMode) => run(async () => {
        const original = summaryOf()
        const mode = requestedMode || original?.projectionValues?.agentPreset
        if (requestedMode) requireMode(requestedMode)
        // Inherit the effective selection without making a new conversation
        // depend on the advisory model catalog being reachable. With no explicit
        // selection, the new session uses the same Host default.
        const model = directory?.store.getSnapshot().current || original?.projectionValues?.modelSelection?.next
        const workspace = workspaceOf(sessionId)
        const target = workspace ? { workspaceId: workspace.workspaceId }
          : original?.cwd ? { cwd: original.cwd } : {}
        const selection = model ? { provider: model.provider, model: model.model,
          ...(model.reasoningEffort === undefined ? {} : { reasoningEffort: model.reasoningEffort }) } : null
        const targetKey = JSON.stringify([target, mode || null, selection])
        if (pendingCreated) {
          const pendingSummary = sessions.list.getSnapshot().byId[pendingCreated.id]
          const pendingState = pendingSummary ? sessions.binding(pendingCreated.id)?.session.getSnapshot() : null
          if (pendingCreated.targetKey !== targetKey || !pendingSummary || !pendingSummary.blank
            || pendingSummary.running || pendingState?.blank === false || pendingState?.running
            || pendingState?.removed || workspaceState().archivedSessionIds.includes(pendingCreated.id)) {
            // This entry may now be somebody's active conversation. Forget the
            // retry handle without deleting or rewriting any existing session.
            pendingCreated = null
          }
        }
        if (!pendingCreated) {
          const id = await sessions.create(target)
          pendingCreated = { id, targetKey, mode, model: selection }
        }
        const pending = pendingCreated
        const id = pending.id
        try {
          assertActive()
          if (pending.mode) unwrap(await ctx.remote.agentPresets.select(id, pending.mode), '设置新对话模式')
          assertActive()
          if (pending.model) await ctx.modelDirectories.directoryFor(id).select({ ...pending.model })
          assertActive()
          sessions.open(id)
          if (pendingCreated === pending) pendingCreated = null
        } catch (caught) {
          // Keep the old conversation selected until the new one is fully
          // configured. Retrying the same selection reuses this blank entry.
          if (caught?.name !== 'AbortError') throw new Error(`新对话已创建，但尚未打开：${caught?.message || '配置失败'}。再次执行相同操作将重试此对话。`)
          throw caught
        }
      })
      for (const [source, store] of [['sessions', sessions.list], ['workspaces', ctx.workspaces?.list], ['session', session], ['models', directory?.store]]) {
        if (store?.subscribe) cleanups.push(store.subscribe(() => requestEmit(source)))
      }
      const controls = {
        getState, refresh, createSession,
        setPerformanceMonitor(value) { monitor = value },
        performanceStats() { return { ...metrics, sources: { ...metrics.sources }, pending: frame !== null || timer !== null } },
        subscribe(listener) { listeners.add(listener); return () => listeners.delete(listener) },
        openSession: (id) => run(async () => {
          if (!getState().sessions.some(item => item.id === id)) throw new Error('该对话已不可用，请刷新会话列表')
          sessions.open(id)
        }),
        selectMode: (id) => run(async () => {
          requireMode(id)
          const current = getState()
          if (!current.blank || current.running) throw new Error('当前对话已经开始，请使用所选模式新建对话')
          unwrap(await ctx.remote.agentPresets.select(sessionId, id), '切换模式')
          assertActive()
          await sessions.refresh()
        }),
        selectModel: (id) => run(async () => {
          if (!directory) throw new Error('模型目录尚未就绪')
          const choice = modelOptions().find(item => item.id === id)
          if (!choice) throw new Error('该模型暂不可用，请刷新模型列表')
          const current = directory.store.getSnapshot().current
          const effort = current?.reasoningEffort
          const reasoningEffort = choice.reasoning?.efforts?.some(item => item.id === effort) ? effort : choice.reasoning?.defaultEffort
          await directory.select({ provider: choice.provider, model: choice.model,
            ...(reasoningEffort ? { reasoningEffort } : {}) })
        }),
        cancel: () => run(async () => {
          if (!session?.cancel) throw new Error('当前会话尚未连接')
          unwrap(await session.cancel(), '停止当前任务')
        }),
        dispose() { disposed = true; cancelPending(); monitor = undefined; pendingCreated = null; for (const cleanup of cleanups) cleanup(); listeners.clear() },
      }
      return controls
    }
    function RhineControl({ useChat, useSession, sessionId }) {
      const [skin, setSkinValue] = useState(activeSkin)
      const [open, setOpen] = useState(activeSkin === 'rhine-lab')
      const trigger = useRef(null)
      const workbench = useRef(null)
      const previousSources = useRef({ sessionId, sources: [] })
      const snapshotDiagnostics = useMemo(() => createRhineSnapshotDiagnostics(), [sessionId])
      const snapshotSelector = useMemo(() => createRhineSnapshotSelector(snapshotDiagnostics), [snapshotDiagnostics])
      const serialized = useChat ? useChat(snapshotSelector) : ''
      const sessionState = useSession ? useSession((snapshot) => JSON.stringify({
        running: Boolean(snapshot.running), hasMoreHistory: Boolean(snapshot.hasMore),
        loadingHistory: Boolean(snapshot.loadingOlder), error: snapshot.lastAgentError
          || (snapshot.promptError?.op === 'send' ? snapshot.promptError.error?.message : '') || '',
        awaitingFirstTurn: Boolean(snapshot.awaitingFirstTurn || snapshot.pendingSubmissions?.some(item => item.placement === 'transcript')),
      })) : '{}'
      const snapshot = useMemo(() => snapshotDiagnostics.measure('host-snapshot-derive', () => {
        const next = snapshotSelector.read(serialized)
        const previous = previousSources.current
        const reuseSources = previous.sessionId === sessionId && previous.input === next.sources
        snapshotDiagnostics.cacheEvent('sourceMerge', reuseSources)
        const sources = reuseSources ? previous.sources : snapshotDiagnostics.measure('host-snapshot-merge',
          () => mergeArchiveSources([...(previous.sessionId === sessionId ? previous.sources : []), ...next.sources]))
        previousSources.current = { sessionId, sources, input: next.sources }
        const lifecycle = JSON.parse(sessionState)
        // Chat's durable turn/end outranks the coarser live running bit, whose
        // control frame can arrive later. A newly submitted first turn still
        // takes precedence while its durable user/turn nodes are on the way.
        const ended = ['completed', 'interrupted', 'error'].includes(next.outcome) && !next.running
        const running = Boolean(lifecycle.awaitingFirstTurn || !ended && (lifecycle.running || next.running))
        const error = ended ? next.error : lifecycle.error || next.error
        const outcome = running ? 'running' : ended ? next.outcome : lifecycle.error ? 'error' : next.outcome || 'unknown'
        const combined = { ...next, ...lifecycle, error,
          sessionId: sessionId || '', sources, running, outcome }
        return { ...combined, phase: rhinePhase(combined) }
      }), [serialized, sessionId, sessionState, snapshotDiagnostics, snapshotSelector])
      const latestSnapshot = useRef(snapshot)
      latestSnapshot.current = snapshot
      useEffect(() => {
        const listener = (value) => {
          setSkinValue(value)
          setOpen(value === 'rhine-lab')
        }
        rhineSkinListeners.add(listener)
        return () => rhineSkinListeners.delete(listener)
      }, [])
      useEffect(() => { workbench.current?.update(snapshot) }, [snapshot])
      useEffect(() => {
        if (!open || skin !== 'rhine-lab' || !sessionId) return undefined
        const ctx = clientContext
        const api = callApi
        const signal = clientAbortController?.signal
        const overlay = document.createElement('div')
        overlay.className = 'prts-rhine-overlay'
        overlay.setAttribute('role', 'dialog')
        overlay.setAttribute('aria-modal', 'true')
        overlay.setAttribute('aria-label', '莱茵生命资料馆')
        overlay.tabIndex = -1
        const status = document.createElement('div')
        status.className = 'prts-rhine-loading'
        const message = document.createElement('p')
        message.textContent = 'RHINE LAB / 正在打开资料馆'
        const close = document.createElement('button')
        close.textContent = '返回对话'
        close.onclick = () => setOpen(false)
        status.append(message, close); overlay.append(status)
        document.body.append(overlay)
        let disposed = false
        let instance = null
        let hostControls = null
        let removeFocusTrap = installDialogFocusTrap(overlay, {
          returnFocus: trigger, isSuspended: isHostSettingsOpen,
        })
        const visible = () => instance?.setActive(!document.hidden)
        ...
        const cleanup = () => {
          ...
          removeFocusTrap()
          hostControls?.dispose()
          instance?.dispose()
          if (workbench.current === instance) workbench.current = null
          if (window.rhineWorkbench === instance) window.rhineWorkbench = null
          overlay.remove()
          rhineCleanups.delete(cleanup)
        }
        rhineCleanups.add(cleanup)
        void loadRhineBundle().then((runtime) => {
          if (disposed || clientContext !== ctx || signal?.aborted) return
          status.remove()
          hostControls = ctx.sessions?.list?.getSnapshot ? createRhineHostControls(ctx, sessionId) : null
          instance = runtime.mountRhineWorkbench(overlay, {
            assetBase: RHINE_BASE, snapshot: latestSnapshot.current, snapshotDiagnostics,
            ...(hostControls ? { host: hostControls } : {}),
            api: (endpoint, payload, requestSignal) => api(endpoint, payload, requestSignal || signal),
            askAgent: (text) => submitRhineQuestion(ctx, sessionId, text, signal),
            loadHistory: async () => {
              const session = ctx?.sessions?.binding?.(sessionId)?.session
              if (!session?.loadOlder) throw new Error('当前会话无法加载更早记录')
              await session.loadOlder()
            },
            close: () => setOpen(false),
            openSettings: () => openHostSettings(ctx),
            isSettingsOpen: isHostSettingsOpen,
          })
          workbench.current = instance
          // Expose the live workbench for diagnostics (bin/measure-rhine-console.js).
          window.rhineWorkbench = instance
          visible()
          removeFocusTrap()
          removeFocusTrap = installDialogFocusTrap(overlay, {
            returnFocus: trigger, isSuspended: isHostSettingsOpen,
          })
        }).catch((error) => {
          if (disposed) return
          if (!status.isConnected) overlay.append(status)
          message.textContent = error?.message || '资料馆加载失败，请关闭后重试'
        })
        return cleanup
      }, [open, skin, sessionId])
      if (skin !== 'rhine-lab') return null
      return h('button', { type: 'button', className: 'prts-rhine-trigger', ref: trigger,
        'aria-label': '打开莱茵生命资料馆', onClick: () => setOpen(true) },
      '▥ 资料馆', snapshot.sources.length ? ` · ${snapshot.sources.length}` : '')
    }


    /* ---- 插件入口：Settings → Plugins 的「PRTS 语料」tab ---- */
    exports.__sceneStateForTest = { buildSceneSnapshotModel, sceneSnapshotSignature,
      collectSnapshotEvidence, buildEvidence, readEvidenceSource, installDialogFocusTrap }
    exports.__rhineStateForTest = { buildRhineSnapshot, mergeArchiveSources, rhineSnapshotSignature, submitRhineQuestion, RhineControl, createRhineHostControls, createRhineSnapshotDiagnostics, createRhineSnapshotSelector }
    exports.__skinStateForTest = {
      setSkin, writeSkinConfig, SkinCard, PrtsSection, openHostSettings, isHostSettingsOpen,
      finishAicBoot: () => aicBootDone(),
      getActiveSkin: () => activeSkin,
      getHostSkinSyncGeneration: () => hostSkinSyncGeneration,
    }
    // Remote namespaces are separate Cordis services. A new model directory
    // also resolves remote.session through this caller's tracked context.
    exports.inject = ['slots', 'connection', 'theme', 'sessions', 'remote', 'remote.agentPresets', 'remote.session', 'workspaces', 'modelDirectories']
    ...
    exports.apply = (ctx) => {
      ...
      const instanceCallApi = async (endpoint, payload, signal = controller.signal) => {
        const response = await fetch('/api/prts-corpus/rpc', {
          method: 'POST',
        ...
        if (!response.ok) throw new Error(`PRTS 服务请求失败（HTTP ${response.status}）`)
        const result = await response.json()
        if (typeof result?.ok !== 'boolean') throw new Error('PRTS 服务返回了无效响应')
        if (!result.ok) throw new Error(result.error?.message || 'PRTS 服务请求失败')
        return result.value
      }
      ...
      // DSH omits the header utilities in a blank session. Keep the workbench
      // on the resident composer so it opens before the first prompt and keeps
      // the same instance when that session gains conversation content.
      ctx.effect(() => ctx.slots.inject('conversation.input.left',
        () => ctx.slots.register(
          { name: 'conversation.input.left', id: 'prts-rhine', order: 17 },
          RhineControl)), 'prts-corpus: rhine archive composer control')
