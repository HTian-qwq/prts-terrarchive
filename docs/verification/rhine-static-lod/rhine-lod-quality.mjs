...
const output=process.env.PRTS_RHINE_LOD_OUTPUT || `/tmp/rhine-lod-quality-${label}`,url=process.env.PRTS_RHINE_PREVIEW_URL || 'http://127.0.0.1:4177/';
...
try{
 ...
 const fixture=JSON.parse(await readFile(new URL('./sources.json',import.meta.url),'utf8'));
 ...
 await page.route('**/rhine/rhine.js',async route=>{const bytes=label==='baseline'?await readFile(new URL('./baseline-rhine.js',import.meta.url)):await(await route.fetch()).body();report.bundle={bytes:bytes.length,sha256:createHash('sha256').update(bytes).digest('hex')};await route.fulfill({body:bytes,contentType:'text/javascript'});});
 ...
 await page.addInitScript(()=>{
  const renderers=[],targets=new Map(),frames=[],scenes=[],trajectory=[];let rendererResizeCalls=0;
