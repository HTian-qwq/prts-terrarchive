import * as THREE from '/home/cloudstack/ai-data/prts.chat/prts-terrarchive/node_modules/three/build/three.module.js';
import ts from '/home/cloudstack/ai-data/prts.chat/prts-terrarchive/node_modules/typescript/lib/typescript.js';
import {readFile,writeFile} from 'node:fs/promises';
import {fileURLToPath} from 'node:url';
const evidence=fileURLToPath(new URL('.',import.meta.url));
const root='/home/cloudstack/ai-data/prts.chat/prts-terrarchive';
async function moduleAt(path){let src=ts.transpileModule(await readFile(path,'utf8'),{compilerOptions:{target:ts.ScriptTarget.ES2022,module:ts.ModuleKind.ESNext}}).outputText;src=src.replace('from "three"',`from "file://${root}/node_modules/three/build/three.module.js"`).replace('from "./array-occlusion.ts"',`from "file://${root}/ui/rhine/original/array-occlusion.ts"`);return import(`data:text/javascript;base64,${Buffer.from(src).toString('base64')}`);}
...
function setup(Type,record,enabled){
 const source=record.arrays.find(m=>m.name==='Optical_Diffuser');const v=new Type(288),batches=record.stats.arrayVisibility.batches.map(meta=>{
  ...
  mesh.userData.occlusionCull=enabled&&(mesh.name.includes('_Bottom')||mesh.name==='Array_Interior_Lettering');return mesh;
 });
 for(const low of batches.filter(m=>m.name.endsWith('_LOD1'))){const high=batches.find(m=>m.name===low.name.replace(/_LOD1$/,''));v.addDetailLOD(high,low,.12,18,22);}
 if(enabled)v.setOpaqueSubstrate(new THREE.PlaneGeometry(4.788000106811523,3.4579999446868896).translate(0,1.8600000143051147,-.03800000250339508));
...
await writeFile(process.env.PRTS_RHINE_CPU_OUTPUT||'/tmp/rhine-occlusion-cpu-replay.json',JSON.stringify(output,null,2));console.log(output.records.map(r=>({view:r.view,label:r.label,medianMs:r.medianMs,p95Ms:r.p95Ms,occlusion:r.stats.occlusion})));
