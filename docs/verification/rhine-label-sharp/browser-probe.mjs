...
try{
 const page=await browser.newPage({viewport:{width:1200,height:750},reducedMotion:'reduce'});page.setDefaultTimeout(180000);page.on('pageerror',e=>report.errors.push(e.message));page.on('console',e=>{if(e.type()==='error')report.errors.push(e.text().slice(0,2000));});
 ...
 await settle();report.array=await page.evaluate(()=>({...window.rhineWorkbench.stats(),surface:JSON.parse(document.querySelector('.rhine-scene').dataset.renderQuality)}));
