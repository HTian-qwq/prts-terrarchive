...
const sources=Array.from({length:167},(_,i)=>({id:`qa-${i+1}`,title:i===130?'远端档案 · 跨架定位测试':`${titles[i%5]} · 资料 ${i+1}`,kind:['character_profile','story','entity_profile','timeline','reviewed_wiki'][i%5],origin:i%5===4?'cloud':'local',state:['found','read','cited'][i%3],saved:i%3!==1,documentId:`qa-doc-${i+1}`,dataVersion:'qa',excerpt:'这是用于界面验证的示例资料：保留完整标题、阅读状态和资料定位。内容不作为剧情结论。'}));
