...
`$DSH_HOME/.agent-presets/prts` 创建「PRTS 模式」预设。`npx` 启动的安装器会把与
自身相同的精确版本加入 profile，避免安装过程中混入其他版本。
...
按完整自然语言标题读取原文；标题使用「活动 · 章节代码 · 篇名 · 行动前后」格式
（如 `骑兵与猎人 / 日正当中 / 行动前`），不使用内部 ID、ref 或路径。
...
- 下载与版本检查**全部优先走 ModelScope 镜像**（含最新版解析），PRTS.chat 本站仅在
  ModelScope 不可达时回退；下载文件写入前校验大小和 SHA-256，同一 release 的并发
  准备由跨进程锁合并
...
| `downloadReleaseId` | 当前 pin 版本 | 设置页显式下载的目标版本 |
...
分片按需读取并由 `cacheShards` 限制缓存数量。Node/V8 可能保留已申请的堆页，操作
系统看到的常驻内存不会在查询结束后立即回落；若需显著压低常驻内存，应把索引迁到
SQLite 等磁盘索引，而不是让首次请求承担全量初始化。
...
src/
  ...
  read.js                corpus_read 执行层（around/section/document/activity）
  timeline.js            timeline_search 执行层（别名裂变、年份交集、出处反查）
  store.js               资料包只读访问层（索引、trigram 倒排、分片 LRU）
...
- **语料数据**：完整语料不随 Git 仓库或 npm 包分发；用户通过插件从 ModelScope 下载的资料适用对应数据集页面标明的许可证、来源声明和使用条款，不因本项目采用 MIT License 而改变。
