...
「资料馆」。证据白板、检索阵列和十二槽档案架位于同一个三维场景，底部导航控制镜头平移。
证据板目前提供手动场景预览：图钉固定纸片，拖动整理位置，用红线连接线索；可从搜索结果或阅读器置入资料。
板卡按会话保存在当前浏览器，初次打开的六张布局示例可清空；证据板尚未接入 Agent。
主界面可以直接向 Agent 提问，真实检索状态驱动阵列运动；实际读取或引用触发抽取和归位。
资料按抵达顺序入架，分别标记查得、已读、已引用，保持用户的翻阅位置。
也可以自行搜索、收藏资料、选择原文摘录，再将问题和摘录交给当前 Agent。

「调查记录」展示工具返回内容；阅读器分别标出搜索相关位置和 Agent 实际读过的范围。
原文绑定资料版本，云端无本地定位的资料显示其返回片段。旧会话可以加载更早记录。
长文分页、档案架分页、调查报告、减少动态效果和无 WebGL 时的文字阅读模式均可用。
...
功能边界、构建和浏览器检查见 [开发说明](docs/rhine-lab.md)。
...
实体检索提示会在 `prts-retrieval` Skill 加载后，按当前用户问题写入 DSH 动态上下文快照；新快照会取代旧快照。PRTS 段只列出当前搭载的资料模块、当前问题命中的规范实体与游戏归属，以及与本轮实体相关的再旅者关系。工具调用标题会同时标明本地或云端资料及检索的游戏范围。
...
- **莱茵生命资料馆**：保留 RhineLabUI 原版循环档案阵列，加入手动证据白板、Agent 提问、调查状态、十二槽档案架、阅读和摘录。操作与构建见 [资料馆说明](docs/rhine-lab.md)。
