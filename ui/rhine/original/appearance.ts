...
export class CardAppearance {
  ...
  prepare(group: THREE.Group) {
    for (const child of group.children) {
      ...
      if (!palette) continue;
      // Shelf borders use the array palette at both appearance endpoints.
      const mat = (mesh.userData.useArrayMaterial && palette.low ? palette.low : palette.high).clone();
  ...
  apply(group: THREE.Group, value: number) {
    for (const child of group.children) {
      ...
      const high = mesh.userData.useArrayMaterial && low ? low : palette.high;
      if (!low) continue;
