...
export const qualityPresets = {
  performance: {
    // Keep thin cassette rims at native CSS resolution. Save work through
    // lighting/postprocessing instead of enlarging an undersampled image.
    scale: 100,
    pixelRatio: 1,
    antialias: "smaa",
    shadows: 1024,
...
/** Upgrade the old complete performance preset without changing custom values. */
export function restoreQuality(value: unknown): RenderQuality {
  const legacyPerformance = { ...qualityPresets.performance, scale: 80, antialias: "off" };
  if (value && typeof value === "object" &&
    Object.entries(legacyPerformance).every(([key, expected]) =>
      (value as Record<string, unknown>)[key] === expected)) {
    return { ...qualityPresets.performance };
  }
