...
export class ModelViewer {
  ...
  private unregisterPerformance?: () => void;
  ...
  constructor(
    ...
    private performanceProbe?: RenderPerformanceCapture,
  ) {
  ...
  update(time: number, rafTime: number | null = null) {
    if (!this.isOpen) return;
  ...
  /** Host plugin teardown: retire pending loads and this independent WebGL context. */
  dispose() {
    this.request++;
    this.transitionId++;
    this.isOpen = false;
