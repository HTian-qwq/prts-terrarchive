...
export class ArrayOcclusion {
  ...
  hidden(bounds: THREE.Box3, matrix: THREE.Matrix4, index: number) {
    ...
    const m = matrix.elements, h = this.half, v = this.view.elements;
    // OBB support along camera Z gives the nearest corner depth directly.
    // The five occluder planes below already prove screen coverage; projecting
    // all eight corners again adds work without strengthening that proof.
    const depthRadius = Math.abs(v[2] * m[0] + v[6] * m[1] + v[10] * m[2]) * h.x
      + Math.abs(v[2] * m[4] + v[6] * m[5] + v[10] * m[6]) * h.y
      + Math.abs(v[2] * m[8] + v[6] * m[9] + v[10] * m[10]) * h.z;
    if (-this.point.z - depthRadius <= this.near + margin) return false;
